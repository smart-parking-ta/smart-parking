...
contract SmartParking {
  ...
  function getUserInfo(uint256 _user_id) public onlyAdmin view returns (
    uint256 user_id, uint256 balance, string memory plate_number, string memory nik, string memory username, string memory phone_number, uint256[] memory order_id){
      return (users[_user_id].user_id, users[_user_id].balance, users[_user_id].plate_number, users[_user_id].nik, users[_user_id].username, users[_user_id].phone_number, users[_user_id].order_id);
  }
  ...
  function userRegister(uint256 _user_id, string memory _plate_number, string memory _nik, string memory _username, string memory _phone_number) public onlyAdmin {
